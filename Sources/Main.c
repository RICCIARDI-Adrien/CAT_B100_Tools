...
int main(int argc, char *argv[])
{
	...
	for (i = 2; i < argc; i++)
	{
		...
		// MAIN_COMMAND_GET_PHONE_BOOK
		else if (strcmp(argv[i], "get-phone-book") == 0)
		{
			Command = MAIN_COMMAND_GET_PHONE_BOOK;
			break;
		}
		// MAIN_COMMAND_GET_ALL_MMS
	...
	switch (Command)
	{
		...
		case MAIN_COMMAND_GET_PHONE_BOOK:
			if (PhoneBookDownloadAll(Serial_Port_ID) != 0)
			{
				printf("Error : could not retrieve the phone book content.\n");
				goto Exit;
			}
			printf("The phone book content was successfully retrieved from the phone.\n");
			break;
